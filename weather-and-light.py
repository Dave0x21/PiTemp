...
def db_write():
    global corr_temperature, corr_humidity, light, pressure
    ...
    time.sleep(10)  # Sleep some seconds to adjust measurements
    while True:
        now = datetime.now()
        now = datetime.strftime(now, "%d-%m-%Y %H:%M")
        temp = float("{:.1f}".format(corr_temperature))
        measurement = (now, temp, int(corr_humidity),
                       int(light), int(pressure), trend)
...
while True:
    ...
    img = overlay_text(background, (0 + margin, 0 + margin),
                       time_string, font_lg)
    ...
    img = overlay_text(img, (WIDTH - margin, 18),
                       light_string, font_lg, align_right=True)
    spacing = font_lg.getsize(light_string.replace(",", ""))[1] + 1
    light_desc = describe_light(light).upper()
    img = overlay_text(img, (WIDTH - margin - 1, 18 + spacing),
                       light_desc, font_sm, align_right=True, rectangle=True)
    ...
    img = overlay_text(img, (WIDTH - margin, 48),
    ...
    img = overlay_text(img, (WIDTH - margin - 1, 48 + spacing),
                       pressure_desc, font_sm, align_right=True, rectangle=True)
